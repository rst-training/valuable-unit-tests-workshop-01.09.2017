...
class DiscountServiceTest extends TestCase
{
    ...
    public function returns_price_discounted_by_15_percent_if_at_least_10_early_bird_seats_are_bought()
    {
        ...
        $configuration->expects($this->at(0))
            ...
            ->with(AtLeastTenEarlyBirdSeatsDiscountStrategy::class)
            ->willReturn(true);
        $configuration->expects($this->at(1))
            ->method('isEnabledForSeat')
            ->with(FreeSeatDiscountStrategy::class)
            ->willReturn(false);
    ...
    public function returns_price_discounted_by_15_percent_if_at_least_10_early_bird_seats_are_bought_simple()
    {
        ...
        $seat = new Seat('test3', 10);
        $this->assertEquals(85, $strategy->calculate($seat, 10));
    }
}
